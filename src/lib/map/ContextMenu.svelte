...
<div class="context-menu" style="left: {position.x}px; top: {position.y}px;"
	...
	<button class="ctx-item" onclick={() => { ms.startDesignation(namingId, 'cue'); onclose(); }}>
		<span class="designation-dot-sm" style="background: var(--ccs-cue)"></span> cue
	</button>
	<button class="ctx-item" onclick={() => { ms.startDesignation(namingId, 'characterization'); onclose(); }}>
		<span class="designation-dot-sm" style="background: var(--ccs-char)"></span> characterization
	</button>
	<button class="ctx-item" onclick={() => { ms.startDesignation(namingId, 'specification'); onclose(); }}>
		<span class="designation-dot-sm" style="background: var(--ccs-spec)"></span> specification
	</button>
	<div class="ctx-separator"></div>
	...
	{#if ms.isPrimary && onunsituate && node && node.mode !== 'relation' && ms.isPlaced(node)}
		<button class="ctx-item" onclick={() => { onunsituate(namingId); onclose(); }}
			title="Return this naming to the Unplaced panel — revise the placement">
			Return to Unplaced
		</button>
	{/if}
