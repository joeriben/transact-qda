...
function buildElkGraph(
	...
	const graph: ElkNode = {
		...
		layoutOptions: {
			...
			// Boxes are real card dimensions (nodeSize.ts), so spacing is true
			// visual distance — 120 was compensation for the fictional 50px height.
			'elk.spacing.nodeNode': '48',
			'elk.spacing.edgeNode': '40',
			// Coding-bridge cues arrive unconnected: pack disconnected components
			// compactly instead of stringing them out.
	...
	// Elements as regular nodes — real rendered card dimensions, not fictions.
	// ELK packing 50px-tall boxes while cards render 3-4× taller was the
	// overlap bug on dense maps.
	for (const el of elements) {
		allNodeIds.add(el.naming_id);
		graph.children!.push({
			id: el.naming_id,
			...estimateNodeSize(el.inscription || ''),
		});
	}
	...
	for (const s of silences) {
		graph.children!.push({
			id: s.naming_id,
			...estimateNodeSize(s.inscription || '', false),
		});
	}
