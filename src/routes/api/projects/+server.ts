...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	if (body.action === 'duplicate') {
		...
		// Scope to the caller's membership: a user must not duplicate (and thereby
		// read) a project they do not belong to. Checked here for a clean 404;
		// the in-transaction SELECT below re-scopes as defense in depth.
		if (!(await isProjectMember(sourceProjectId, userId))) {
