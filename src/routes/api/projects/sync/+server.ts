	...
	try {
		switch (action) {
			...
			case 'load': {
				...
				// Only a member of the archived project may load it. Membership comes
				// from the archive itself (project_members.copy); loading restores it
				// as-is — no ownership/authorship reassignment. This stops a researcher
				// from pulling a colleague's project out of the shared projekte/ folder.
				const members = await readArchivedMemberIds(dir);
				if (!members.includes(locals.user.id)) {
					return json({ error: 'Not your project' }, { status: 403 });
				}
