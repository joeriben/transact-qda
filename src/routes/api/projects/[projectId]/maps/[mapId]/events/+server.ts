...
import { getMap } from '$lib/server/db/queries/maps.js';

// SSE endpoint for real-time AI notifications on a map.
// Clients subscribe by opening a persistent GET connection.
export const GET: RequestHandler = async ({ params }) => {
	const { mapId, projectId } = params;

	// Scope the map UUID to this project (must not subscribe to a foreign map's stream)
