...
<script lang="ts">
	...
	async function deleteDiskProject(slug: string) {
		const typed = prompt(`Permanently delete the "${slug}" archive?\nType the project name to confirm:`);
		if (typed !== slug) {
		...
		try {
			...
			const result = await res.json();
			if (!res.ok) throw new Error(result.error);
			syncMessage = `Deleted "${slug}" archive`;
			await invalidateAll();
		} catch (e: any) {
			syncMessage = e.message;
		} finally {
	...
	// Load a native .tqda.zip archive from the user's computer
	async function importNative(e: Event) {
		const input = e.target as HTMLInputElement;
		const file = input.files?.[0];
		if (!file) return;
		importing = true;
		importError = null;
		importStatus = `Loading ${file.name} ... this may take a while for large projects.`;
		try {
			const form = new FormData();
			form.append('file', file);
			const res = await fetch('/api/projects/native-import', { method: 'POST', body: form });
			const result = await res.json();
			if (res.ok) {
				importStatus = 'Loaded. Opening project...';
				goto(`/projects/${result.projectId}`);
			} else {
				importError = result.error || 'Native import failed';
			}
		} catch (e: any) {
			importError = e.message || 'Native import failed';
		} finally {
			importing = false;
			if (importError) importStatus = null;
			input.value = '';
...
<div class="projects-page">
	<div class="header">
		...
		<div class="header-actions">
			<label class="btn-secondary" class:disabled={importing} title="Load a project from a native .tqda.zip file on your computer">
				{importing ? 'Loading...' : 'Load from file'}
				<input type="file" accept=".zip,.tqda.zip" onchange={importNative} hidden disabled={importing} />
			</label>
	...
	<!-- Unloaded projects (archived in the projects directory, not in DB) -->
	<h2 class="section-label" id="on-disk">Archives (not loaded)</h2>
		...
		<div class="project-grid">
			{#each unloadedDirs as dir}
				<div class="project-card project-card-unloaded">
					...
					<div class="card-actions">
						<button class="action-btn" title="Load from archive"
							onclick={() => loadProject(dir.slug)} disabled={syncing}>
							📥
						</button>
						<button class="action-btn action-delete" title="Delete archive"
