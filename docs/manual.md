...
**Adding elements** creates a naming with an initial designation of `cue` and an entity-appearance on the map. At this stage, nothing is determined — the element is a pointing gesture, a registered signal. The map's designation profile (aggregate counts per CCS stage) shows predominantly cues: the map is in its messy phase.

**Relating elements** creates a participation-naming that bonds two namings. The relation itself gets an inscription ("constrains," "enables," "constitutes"), a designation, and its own stack. Because the relation is a naming, it can be related to other namings — including other relations. Clarke's relational analysis ("what is the nature of the relationship?") produces namings, not annotations on edges.
